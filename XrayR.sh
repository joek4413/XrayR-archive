...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/joek4413/XrayR-archive/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/joek4413/XrayR-archive/main/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/joek4413/XrayR-archive/main/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/joek4413/XrayR-archive ---
  ${green}0.${plain} 修改配置
————————————————
